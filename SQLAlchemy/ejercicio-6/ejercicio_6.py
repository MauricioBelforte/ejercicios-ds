"""
Ejercicio 6 — Modelo Estudiante (Many-to-Many)

Crear el modelo `Estudiante` (`id`, `nombre`, `legajo`).

Como un estudiante cursa muchas materias y una materia tiene muchos alumnos,
definir la tabla asociativa llamada `Inscripcion` para conectar `Estudiantes`
y `Cursos`.
...
if __name__ == "__main__":
    ...
    with Session(engine) as session:
        ...
        inscripcion_1 = Inscripcion(estudiante=estudiante_ana, curso=curso_programacion)
        inscripcion_2 = Inscripcion(estudiante=estudiante_ana, curso=curso_matematica)
        inscripcion_3 = Inscripcion(estudiante=estudiante_juan, curso=curso_programacion)
        inscripcion_4 = Inscripcion(estudiante=estudiante_lucia, curso=curso_fisica)
        inscripcion_5 = Inscripcion(estudiante=estudiante_lucia, curso=curso_programacion)
    ...
    with Session(engine) as session:
        ...
        if estudiante:
            ...
            for inscripcion in estudiante.inscripciones:
                print(f" └─ 📚 {inscripcion.curso.titulo} ({inscripcion.curso.creditos} crs.)")
    ...
    with Session(engine) as session:
        ...
        if curso:
            ...
            for inscripcion in curso.inscripciones:
                print(f" └─ 👤 {inscripcion.estudiante.nombre} (Legajo: {inscripcion.estudiante.legajo})")